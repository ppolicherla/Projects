...
#include "curl.h"
//inclusion of my RC4 program for encryption and decryption 
#include "RC4enc/main.cpp"
...
int FdEntity::Load(off_t start, size_t size)
{
  ...
  if(0 < pagelist.GetUnloadedPages(unloaded_list, start, size)){
    for(fdpage_list_t::iterator iter = unloaded_list.begin(); iter != unloaded_list.end(); ++iter){
      ...
      //Pavan added code to call the encryption function to run as the file is being loaded
      encdec(fd);
...
int FdEntity::RowFlush(const char* tpath, bool force_sync)
{
  ...
  //Pavan added code to encode the file before it is locked
  encdec(fd);
